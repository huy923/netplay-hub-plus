  ...
  .handler(async ({ data }) => {
    ...
    const invoice = await prisma.invoice.create({
      data: {
        ...
        status: data.paymentMethod === "cash" ? "Đã thanh toán" : "Chờ",
        time,
        items: {
          create: allItemsData,
        },
    ...
    const paymentStatus = data.paymentMethod === "cash" ? "success" : "pending";
    ...
    await prisma.transaction.create({
      data: {
        type: data.paymentMethod === "cash" ? "payment_success" : "payment_created",
        amount: totalAmount,
...
export const confirmEndSessionIdle = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    await prisma.machine.update({
      ...
      data: { status: "idle", remaining: null, startedAt: null },
    });
